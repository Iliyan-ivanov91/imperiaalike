<template>
    <div class="max-w-sm w-full bg-white rounded-xl shadow-md p-6 flex flex-col space-y-8 relative pt-12">
        ...
        <h1 class="text-center font-bold text-xl">Create your Account!</h1>
        <form @submit.prevent class="flex flex-col space-y-6">
            ...
            <div class="text-center">
                <button @click="signup()" class="px-6 py-2 bg-green-500 text-white font-bold rounded-md w-full uppercase">
                    <span v-if="!loading">Join now</span>
                    <span v-else><font-awesome-icon :icon="['fas', 'spinner']" class="fa-spin" /> Loading</span>
                </button>
            </div>
        </form>
        <div class="mt-4 py-4">
            <div class="text-center uppercase text-xs text-gray-500">Already have an account? <button @click="$emit('switch')" class="text-green-500 uppercase font-bold">Sign in</button></div>
        </div>
    </div>
</template>
